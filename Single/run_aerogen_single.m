...
u_up = [0;  1; 0.1; 2*pi/(tf+dt)+1e-6];
u_lw = [-20000; -1; 0; -2*pi/(tf+dt)-1e-6];
x_up = [inf;  inf; inf; pi/3; inf; inf;  inf];%; inf];
x_lw = [-inf; -inf;  0; -pi/3; -inf; -inf; -inf];%; -inf];
x0_up = [0;   inf;   inf; pi/3;   0; 0;    0];%; inf];
...
Objective = u(1, 1).*x(2, 1)+abs(u(2, 1)-u(2, end))*100+abs(u(1, 1)-u(1, end))*0.01;
x_dotk = dynamic_single(x(:, end), u(:, end), p);
x_dotk1 = dynamic_single(x(:, 1), u(:, 1), p);
Constraints = [Constraints, x([1, 2, 3, 4, 6, 7], 1) == x([1, 2, 3, 4, 6, 7], end)+dt/2*(x_dotk([1, 2, 3, 4, 6, 7])+x_dotk1([1, 2, 3, 4, 6, 7])),...
...
for m = 1 : gridSz-1
  ...
  Objective = Objective + u(1, m+1).*x(2, m+1)+abs(u(2, m+1)-u(2, m))*100+abs(u(1, m+1)-u(1, m))*0.01; %objective func, max energy
end
