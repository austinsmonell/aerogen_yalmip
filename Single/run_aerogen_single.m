...
x0_up = [0; inf; 100; pi/4; 0; 0; 0];
x0_lw = [0; 0; 1; -pi/4; 0;  0; 0];
...
cyl_idx = [2, 3, 4, 6, 7];
Objective = u(1, 1).*x(2, 1)+sum(abs(u(:, 1)-u(:, end)))*10;
...
for m = 1 : gridSz-1
  ...
  Objective = Objective + u(1, m+1).*x(2, m+1)+sum(abs(u(:, m+1)-u(:, m)))*10; %objective func, max energy
end
